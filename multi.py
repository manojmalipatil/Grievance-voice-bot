...
async def entrypoint(ctx: JobContext):
    ...
    async def select_language(language: Optional[str] = None):
        ...
        if lang_lower == "cannada" or "canada" in lang_lower:
            lang_lower = "kannada"
    ...
    try:
        ...
        await session_1.say(
            "Thank you for calling Samasta Grievance Assistant. Which language would you like to speak? We support English and Hindi.",
    ...
    # ============================================================================
    logger.info("[STAGE 2] Call ending - waiting for farewell to complete...")
    
    # The agent speaks BEFORE calling end_call()
    # We need to wait for:
    # 1. TTS generation to complete
    # 2. Audio to transmit over network
    # 3. Audio to play through speakers
    # 4. Audio buffers to drain
    # 
    # A generous 8-second wait ensures everything completes cleanly
    # This prevents WebSocket errors from closing too early
    logger.info("[STAGE 2] Waiting 12 seconds for complete audio playback...")
